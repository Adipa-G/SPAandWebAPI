...
export class TablePager extends React.Component<TablePagerProps, TablePagerState> {
    ...
    changePage = (pageNumber: number) => {
        ...
        this.setState((prevState: TablePagerState) => {
            let newState: TablePagerState = jQuery.extend(true, {}, prevState);
    ...
    render() {
        ...
        for (var i = 0; i <= (endPage - startPage); i++) {
            ...
            var page = <li key={iCopy} className={startPage + iCopy === this.state.page ? "active" : ""}>
                <a href={startPage + iCopy <= endPage ? "javascript:;" : null}
                    onClick={() => startPage + iCopy <= endPage ? this.changePage(startPage + iCopy) : {}}>
                    {startPage + iCopy}
                </a>
            </li>;
            pages.push(page);
            ...
            <ul className="pagination">
                <li>
                    <a aria-label="Previous"
                        href={isPreviousEnabled ? "javascript:;" : null}
                        onClick={() => isPreviousEnabled ? this.changePage(this.state.page - 1) : {}}>
                ...
                <li>
                    <a aria-label="Next"
                        href={isNextEnabled ? "javascript:;" : null}
                        onClick={() => isNextEnabled ? this.changePage(this.state.page + 1) : {}}>
